...
async def parse_commands_gpt(text: str) -> list[dict]:
    ...
    prompt = (
        ...
        "   Если пользователь просит совершить действие на странице (кликнуть на первое видео, включить 'Мою волну', запустить воспроизведение, нажать кнопку):\n"
        "   Ты должен сгенерировать Python-скрипт с использованием `pyautogui` и `time`!\n"
        "   - Пример для 'включи мою волну' (Яндекс.Музыка):\n"
        "     Скрипт должен открыть вкладку с музыкой (или открыть https://music.yandex.ru/home), подождать 3 секунды для загрузки, "
        "     а затем кликнуть в область 'Моей волны' (примерные координаты: x=300, y=350) или нажать пробел/плей.\n"
        "     Код: \n"
        ...
        "     Скрипт должен кликнуть в область первого видео на главном экране (координаты примерно x=280, y=350 для сетки YouTube в разрешении Full HD):\n"
        "     Код: \n"
        "     import time, pyautogui\n"
        "     pyautogui.click(280, 350)\n"
        "   - Любые другие действия (ввод текста, сложные клики) — переводи в готовый скрипт python и возвращай как run_custom_script.\n"
