...
async def parse_commands_gpt(text: str):
    ...
    prompt = (
        ...
        "4. Включить Мою Волну: {\"command\": \"play_my_wave\", \"args\": {\"target_screen\": \"main|second\"}}\n"
        "   - Вызывай строго эту команду, если пользователь просит 'включи мою волну', 'запусти мою волну', 'включи музыку'!\n"
        "5. Открыть первое видео на YouTube: {\"command\": \"play_first_youtube_video\", \"args\": {\"target_screen\": \"main|second\"}}\n"
        "   - Вызывай строго эту команду, если пользователь просит 'открой первое видео', 'включи первое видео', 'первое видео в рекомендациях'!\n"
        "6. Управление медиа: {\"command\": \"media_key\", \"args\": {\"key\": \"space|next|prev|volume_up|volume_down|mute|scroll_down|scroll_up|f11|close_tab|new_tab\"}}\n"
        "7. Запуск Python скрипта: {\"command\": \"run_custom_script\", \"args\": {\"script_code\": \"код на python\"}}\n"
        "8. Запуск приложений: {\"command\": \"open_app\", \"args\": {\"app_name\": \"калькулятор|блокнот|проводник|паинт|vs code|steam|telegram|discord|браузер\", \"target_screen\": \"main|second\"}}\n"
        "9. Громкость: {\"command\": \"adjust_volume\", \"args\": {\"action\": \"up|down|mute\", \"amount\": число_громкости_по_умолчанию_10}}\n"
        "10. Переместить окно: {\"command\": \"move_window\", \"args\": {\"keyword\": \"ключевое слово из названия окна\", \"target_screen\": \"main|second\"}}\n"
        ...
        "11. Список всех окон: {\"command\": \"list_windows\"} (вызывай, если просят показать открытые окна или процессы)\n\n"
        "КРИТИЧЕСКИЕ ПРАВИЛА ВЫБОРА КОМАНДЫ:\n"
        "- Запуск 'Моей волны' -> 'play_my_wave'!\n"
        "- Клик по первому видео на YouTube -> 'play_first_youtube_video'!\n\n"
        "12. ДИНАМИЧЕСКИЙ СКРИПТ (САМОАДАПТАЦИЯ):\n"
        "   Если пользователь просит совершить действие на странице (кликнуть в другие координаты, нажать специфическую кнопку):\n"
        "   Ты должен сгенерировать Python-скрипт с использованием `pyautogui`, `pygetwindow as gw` и `time`!\n"
        "   Скрипт должен сначала найти окно браузера, активировать его через `win.activate()`, подождать 0.5 секунд, и только потом кликать!\n"
        "   Любые другие действия — переводи в готовый скрипт python с фокусом окна и кликами.\n"
        "13. Выключение/Перезагрузка: {\"command\": \"shutdown\" или \"restart\", \"args\": {\"delay\": секунды_по_умолчанию_10}}\n\n"
